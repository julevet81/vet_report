...
<div class="card p-5 max-w-2xl">
    <h2 class="text-xl font-bold mb-4">Add Branch</h2>
        ...
        <div>
            ...
            <select class="input" name="wilaya_inspectorate_id" required>
                ...
                @foreach($inspectorates as $inspectorate)
                    <option value="{{ $inspectorate->id }}" @selected((string) old('wilaya_inspectorate_id') === (string) $inspectorate->id)>{{ $inspectorate->name }} ({{ $inspectorate->code }})</option>
                @endforeach
            </select>
        </div>
